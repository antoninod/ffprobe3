    ...
    def __init__(self, video_file):
        ...
        try:
            with open(os.devnull, 'w') as tempf:
                subprocess.check_call(["ffprobe", "-h"], stdin=subprocess.devnull, stdout=tempf, stderr=tempf)
        except:
        ...
        if os.path.isfile(video_file):
            ...
            p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.devnull, shell=True)
            self.format = None
            self.created = None
            self.duration = None
            self.start = None
